...
Statistical methods assume that normal data objects are generated by a stochastic model, and that data not following this model are outliers.
...
\begin{enumerate}
    ...
    \item Conclusion: Find threshold were most of the data is located, for example $\mu \pm 3\sigma$ and classify all data outside as outliers
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Distance based methods} find outliers with respect to global dataset
...
\subsubsection{Distance-Based Ouliers}

\begin{itemize}
    \item Specify a ditsance threshold $r>0$ and a fraction threshold $0<\pi\leq 1$
...
\subsubsection{Density-Based Ouliers}
...
The reachability distance is simply the maximum of the distance of two points and the k-distance of the second point, consider it as a smoothing term, a lower distance bound that depends on the neighbourhood density.
...
Local reachability density of a point in comparison to its k neighbours, more specifically, $k$ ratios of the local reachability distance of each point to its neighbouring points are calculated and averaged.
