...
\begin{enumerate}
    ...
    \item If there are still instances but no features any more, the training data s contradictory, noisy, non-deterministic or contains hidden features. In his case, decide according to the more frequent label.
\end{enumerate}
...
The regression tress predict the average value of all instances in a leaf node and are grown like classification trees but with a different splitting criteria. As splitting criteria, they minimise the variance of the values in the same subset.
...
\textbf{Advantages}
\begin{itemize}
    \item Decision trees are simple to understand and interpret for humans
    \item Decision trees can handle both numeric and categorical data
    \item Decision trees require almost no data preparation
    \item Decision trees perform well even on larger datasets
\end{itemize}
...
Mining association rules is unsupervised learning, whereas building decision trees is supervised learning. Where association rules process pure transactions, decision trees incorporate other information, therefore both techniques can be used in a personalized and impersonalized manner.
...
\begin{enumerate}
    ...
    \item You train your favorite learning algorithm on the training set
...
\begin{itemize}
    \item Try a larger set of features
    \item Try a different set of features
    \item Try more complex machine learning algorithms
\end{itemize}
...
Overfitting happens more frequently and is much harder to repair. It occurs cause your model is to sensitive to the training data, you could try to:
\begin{itemize}
    \item Try getting more training data
    \item Try better data cleaning and eliminate outliers
    \item Try a smaller set of features through feature selection
    \item Try early stopping during iterative training
    \item Try tuning the regularization parameter
    \item Try ensemble methods
\end{itemize}
