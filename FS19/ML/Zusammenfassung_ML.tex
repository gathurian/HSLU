...
\begin{minipage}{0.45\textwidth}
    ...
    Therefore: The best projection is the one that minimizes information loss or equivalently that maximizes the variance.
\end{minipage}
...
Take for example a \textbf{mean-centered feature} that always takes the same value (so, the mean). This feature dow not give you any valuable information and can therefore safely be dropped.

Another example are \textbf{several features that essentially say the same}, say the height of a test person in centimeters and in inches. One of the two can be dropped. In the unlikely case that both measurements need to be used, one can easily be calculated.
...
\begin{description}
    \item[Eigenvector: ] A vector that does only stretch or rotate (but noth both) a matrix $A$ when multiplied with the matrix
    \item[Eigenvalue: ] A factor of the eigenvector
\end{description}

\noindent A vector $ v \neq 0$ is called an eigenvector if it satisfies $Av = \lambda v$. $\lambda$ is called the eigenvalue of the eigenvector $v$.

Eigenvectors (and therefore eigenvalues) only exist for \textbf{squared matrices}.o
