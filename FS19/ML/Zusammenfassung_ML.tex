...
\noindent 1 means the data is perfectly correlated, whereas -1 means that the data is perfectly uncorrelated
...
\begin{flalign*}
    &X = [4, 6, 8, 10]& \\
    &Y = [2.3, 4.1, 5.7, 6.9]& \\
    &\bar X = \frac{1}{4} (4 \cdot 6 \cdot 8 \cdot 10) = \doubleunderline{7}& \\
    &\bar Y = \frac{1}{4} (2.3 \cdot 4.1 \cdot 5.7 \cdot 6.9) = \doubleunderline{7.25}& \\
    ...
    &\theta^{}_{1} = \frac{20}{15.4} = \doubleunderline{0.77}& \\
...
\begin{enumerate}
    ...
    \item Compute predictions for products labelled with \textcolor{red}{$?$} based on item similarity (See Equation \ref{eq:sim} on page \pageref{eq:sim})
...
\begin{minipage}{0.45\textwidth}
    ...
    \captionof{figure}{How to measure retained information}
...
As said in the previous page: With almost any dimension reduction, some information will get lost. However, sometimes this does not hold true. This is the case if some data is \textit{redundant}.

\vspace{10px}

\noindent
Take for example a \textbf{mean-centred feature} that always takes the same value (so, the mean). This feature does not give you any valuable information and can therefore safely be dropped. Another example is \textbf{several features that essentially say the same}, say the height of a test person in centimetres and in inches. One of the two can be dropped. In the unlikely case that both measurements are needed, one can easily be calculated from the other.
